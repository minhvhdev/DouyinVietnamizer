...
const baseApi: JobsApi = {
  ...
  listPresetVoices: vi.fn().mockResolvedValue([]),
...
test("shows an actionable backend connection error", async () => {
  ...
  expect(screen.getByText(/kiá»ƒm tra nháº­t kÃ½ hoáº¡t Ä‘á»™ng hoáº·c cáº¥u hÃ¬nh/i)).toBeInTheDocument();
});
...
test("creates a job and shows it on the dashboard", async () => {
  ...
  fireEvent.change(await screen.findByPlaceholderText(/DÃ¡n liÃªn káº¿t video Douyin hoáº·c Bilibili/), {
    target: { value: job.source_url }
  });
  fireEvent.click(screen.getByRole("button", { name: "Táº¡o tiáº¿n trÃ¬nh" }));

  await waitFor(() => expect(screen.getByText(job.source_url)).toBeInTheDocument());
  expect(screen.queryByText("Chi tiáº¿t tiáº¿n trÃ¬nh")).not.toBeInTheDocument();
});
...
test("job details opens only by click and closes without leaving selection active", async () => {
  ...
  expect(await screen.findByText("Chi tiáº¿t tiáº¿n trÃ¬nh")).toBeInTheDocument();

  fireEvent.click(screen.getByLabelText("Close job details"));

  await waitFor(() => expect(screen.queryByText("Chi tiáº¿t tiáº¿n trÃ¬nh")).not.toBeInTheDocument());
...
test("deletes completed jobs from the dashboard", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: `XÃ³a tiáº¿n trÃ¬nh ${job.id}` }));
...
test("shows runtime checks and reruns the smoke test", async () => {
  ...
  fireEvent.click(await screen.findByRole("button", { name: /MÃ´i trÆ°á»ng/ }));
  expect(screen.getByText("Install the tool under vendor/ or add it to PATH, then retry.")).toBeInTheDocument();
  fireEvent.click(screen.getByRole("button", { name: "Cháº¡y thá»­ nghiá»‡m há»‡ thá»‘ng" }));

  await waitFor(() => expect(api.runSmokeTest).toHaveBeenCalled());
  expect(await screen.findByText("ÄÃ£ sáºµn sÃ ng")).toBeInTheDocument();
});
...
test("shows environment setup wizard when runtime is blocked", async () => {
  ...
  expect(await screen.findByText("Thiáº¿t láº­p mÃ´i trÆ°á»ng á»©ng dá»¥ng")).toBeInTheDocument();
});
...
test("runtime panel distinguishes required and optional warnings", async () => {
  ...
  expect(screen.getAllByText("Báº¯t buá»™c").length).toBeGreaterThan(0);
  expect(screen.getByText("Tuá»³ chá»n")).toBeInTheDocument();
  expect(screen.getByText("Cáº£nh bÃ¡o")).toBeInTheDocument();
...
test("shows translation and OmniVoice settings with browser cookie disclosure", async () => {
  ...
  fireEvent.click(await screen.findByRole("button", { name: "CÃ i Ä‘áº·t" }));

  expect(screen.getAllByText("Google Dá»‹ch Miá»…n PhÃ­").length).toBeGreaterThan(0);
...
test("manages Gemini API key pool from settings", async () => {
  ...
  fireEvent.click(await screen.findByRole("button", { name: "CÃ i Ä‘áº·t" }));
  fireEvent.click(screen.getByRole("button", { name: "API Gemini" }));

  expect(await screen.findByText("KhÃ³a API Gemini")).toBeInTheDocument();
  expect(screen.getByText("AIza...7890")).toBeInTheDocument();
  expect(screen.getByDisplayValue("Studio quota 1")).toBeInTheDocument();

  fireEvent.change(screen.getByPlaceholderText(/DÃ¡n khÃ³a API Google AI Studio/i), {
    target: { value: "AIzaSyNewSecret" }
  });
  fireEvent.click(screen.getByRole("button", { name: "ThÃªm khÃ³a Gemini" }));
...
test("saves a pending Gemini key when saving settings", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "LÆ°u cÃ i Ä‘áº·t" }));
  ...
  expect(screen.getByPlaceholderText(/DÃ¡n khÃ³a API Google AI Studio/i)).toHaveValue("");
});
...
test("navigates to Clone Giong tab and lists cloned voices", async () => {
  const clonedVoices = [
    { id: "voice-1", name: "Giá»ng Anh", wav_filename: "v1.wav", wav_path: "/path/v1.wav", created_at: "2026-06-16T12:00:00Z" }
  ];
  const api: JobsApi = {
    ...baseApi,
  ...
  fireEvent.click(await screen.findByRole("button", { name: "Clone Giá»ng" }));

  expect(await screen.findByText("Giá»ng Anh")).toBeInTheDocument();
