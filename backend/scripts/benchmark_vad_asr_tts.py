...
Run:
    python -m scripts.benchmark_vad_asr_tts
    python -m scripts.benchmark_vad_asr_tts --real-env --out benchmark.json
...
from dv_backend.tts_conversion import convert_segments, convert_segments_per_segment, describe as describe_conversion  # noqa: E402
...
def main() -> int:
    ...
    per_segment = None
    try:
        per_segment = convert_segments_per_segment(config, pipeline_module, "bench", None, segments)
    except Exception as error:
        print(f"  per_segment_skipped reason={error.__class__.__name__}: {error}")
    lazy_mix = convert_segments(config, pipeline_module, "bench", None, segments, settings={"tts_conversion_strategy": "lazy_mix"})
    conversion_summary = {
        "mode": "mock_audio",
