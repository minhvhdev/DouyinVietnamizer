"""Persistent on-disk cache for OmniVoice TTS outputs.

Cache key = sha256(voice_id, normalized_text, model, num_step, instruct).
The same input always returns the same cached file, so re-running a job
or dubbing the same video twice becomes instant for any repeated segment.
"""
...
OMNIVOICE_CACHE_VERSION = "v2-lang-vi"
